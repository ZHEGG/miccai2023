...
parser.add_argument('--eval-metric', default='loss', type=str, metavar='EVAL_METRIC',
                    help='Main metric (default: "loss"')
parser.add_argument('--report-metrics', default=['loss'], 
                    nargs='+', choices=['loss'], 
...
def main():
    ...
    # TODO 如果冻结，还有第一层卷积需要训练
    # if args.model == "resnet3d_34_classify":
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if metric_savers is not None:
                for metric in args.report_metrics:
                    # Save the checkpoint for every 10 epoch 
                    if 99<epoch<251 and epoch%10==0:
                        best_metrics[metric]['value'] = eval_metrics[metric]
                        best_metrics[metric]['epoch'] = epoch
                        ckpt_saver = metric_savers[metric]
                        save_path = os.path.join(output_dir,'%s.pth'%epoch)
                        ckpt_saver._save(save_path,epoch,metric=best_metrics[metric]['value'])
    ...
    if best_metric is not None:
        ...
        with open(os.path.join(output_dir, 'summary.csv'),'a',newline='') as csv_file:
            ...
            writer.writerow(['最好loss指标-epoch数'])
...
def train_one_epoch(
    ...
    for batch_idx, item in enumerate(loader):
        ...
        with amp_autocast():
            if args.model == "uniformer_small_IL" or args.model == "uniformer_small_original" or args.model == "resnet3d_34_classify" \
            or args.model == "uniformer_base_original" or args.model == "uniformer_xs_original" or args.model == "uniformer_xxs_original" :
                output,visualizations = model(input)
            elif args.model == "HiFuse_small_phase":
                B,PHASE,VIEW,H,W = input.shape
                input = input.permute(0,2,1,3,4).contiguous()
                input = input.reshape(B*VIEW,PHASE,H,W).contiguous()
                output = model(input)
                output = output.reshape(B,VIEW,-1).contiguous()
                output = torch.mean(output,dim=1)
            elif args.model == "HiFuse_small_view":
                B,PHASE,VIEW,H,W = input.shape
                input = input.reshape(B*PHASE,VIEW,H,W).contiguous()
                output = model(input)
                output = output.reshape(B,PHASE,-1).contiguous()
                output = torch.mean(output,dim=1)
            elif args.model == "uniformer_UniFormer_glb_local":
                input_img = torch.cat([input.unsqueeze(2),global_input.unsqueeze(2)],dim=2)
                output,visualizations = model(input_img)
